...
```sh
mkdir -p nixos-config && cd nixos-config && nix flake init -t github:boydwold/nix-config#starter
```
...
```sh
mkdir -p nixos-config && cd nixos-config && nix flake init -t github:boydwold/nix-config#starter-with-secrets
```
